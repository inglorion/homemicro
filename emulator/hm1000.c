...
static void disas_abs(uint16_t pc, const uint8_t *data) {
  uint8_t op = data[pc];
  uint8_t l = data[(pc + 1) & 0xffff];
  uint8_t h = data[(pc + 2) & 0xffff];
  printf("%04x: %02x %02x %02x %s $%02x%02x\n",
         pc, op, l, h, opnames[op], h, l);
}

static void disas_absx(uint16_t pc, const uint8_t *data) {
  uint8_t op = data[pc];
  uint8_t l = data[(pc + 1) & 0xffff];
  uint8_t h = data[(pc + 2) & 0xffff];
...
static void disas_absy(uint16_t pc, const uint8_t *data) {
  uint8_t op = data[pc];
  uint8_t l = data[(pc + 1) & 0xffff];
  uint8_t h = data[(pc + 2) & 0xffff];
...
static void disas_imm(uint16_t pc, const uint8_t *data) {
  uint8_t op = data[pc];
  int8_t imm = data[(pc + 1) & 0xffff];
  printf("%04x: %02x %02x    %s %d\n",
         pc, op, (uint8_t) imm, opnames[op], imm);
}

static void disas_impl(uint16_t pc, const uint8_t *data) {
  uint8_t op = data[pc];
  printf("%04x: %02x       %s\n", pc, op, opnames[op]);
}

static void disas_ind(uint16_t pc, const uint8_t *data) {
  uint8_t op = data[pc];
  uint8_t l = data[(pc + 1) & 0xffff];
  uint8_t h = data[(pc + 2) & 0xffff];
  printf("%04x: %02x %02x %02x %s ($%02x%02x)\n",
...
static void disas_ix(uint16_t pc, const uint8_t *data) {
  uint8_t op = data[pc];
  uint8_t addr = data[(pc + 1) & 0xffff];
  printf("%04x: %02x %02x    %s ($%x, x)\n",
         pc, op, addr, opnames[op], addr);
}

static void disas_iy(uint16_t pc, const uint8_t *data) {
  uint8_t op = data[pc];
  uint8_t addr = data[(pc + 1) & 0xffff];
  printf("%04x: %02x %02x    %s ($%x), y\n",
         pc, op, addr, opnames[op], addr);
}

static void disas_ni(uint16_t pc, const uint8_t *data) {
  uint8_t op = data[pc];
  printf("%04x: %02x       unknown op %02x\n",
         pc, op, op);
}

static void disas_rel(uint16_t pc, const uint8_t *data) {
  uint8_t op = data[pc];
  int8_t offset = data[(pc + 1) & 0xffff];
  printf("%04x: %02x %02x    %s %04x\n",
         pc, op, (uint8_t) offset,
         opnames[op],
         (pc + 2 + offset) & 0xffff);
}

static void disas_zp(uint16_t pc, const uint8_t *data) {
  uint8_t op = data[pc];
  uint8_t addr = data[(pc + 1) & 0xffff];
  printf("%04x: %02x %02x    %s $%x\n",
         pc, op, addr, opnames[op], addr);
}

static void disas_zpx(uint16_t pc, const uint8_t *data) {
  uint8_t op = data[pc];
  uint8_t addr = data[(pc + 1) & 0xffff];
  printf("%04x: %02x %02x    %s $%x, x\n",
         pc, op, addr, opnames[op], addr);
...
static void disas_zpy(uint16_t pc, const uint8_t *data) {
  uint8_t op = data[pc];
  uint8_t addr = data[(pc + 1) & 0xffff];
  printf("%04x: %02x %02x    %s $%x, y\n",
         pc, op, addr, opnames[op], addr);
...
#define OP(CODE, NAME, MODE, CYCLES) disas_ ## MODE,
typedef void (*disas_func)(uint16_t, const uint8_t*);
static disas_func disas_modes[] = {
#include "ops.inc"
};
#undef OP
